...
export function useFilterSync(): [
  BazzaFiltersState,
  (newFilters: BazzaFiltersState | ((prev: BazzaFiltersState) => BazzaFiltersState)) => void,
] {
  ...
  const setFilters = useCallback(
    (newFilters: BazzaFiltersState | ((prev: BazzaFiltersState) => BazzaFiltersState)) => {
      // Handle functional updates by resolving the function with current filters
      const resolvedFilters = typeof newFilters === 'function' ? newFilters(filtersFromUrl) : newFilters;
