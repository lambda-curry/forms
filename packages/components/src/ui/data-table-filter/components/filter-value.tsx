      ...
      {hasOptionIcons &&
        take(selected, 3).map(({ value, icon }) => {
          ...
          return isValidElement(Icon) ? (
            ...
            <Icon key={value} className="h-4 w-4" />
          )
        })}
...
export function FilterValueDateDisplay<TData>({
  ...
  if (filter.values.length === 0) return <Ellipsis className="h-4 w-4" />
...
function OptionItem({ option, onToggle }: OptionItemProps) {
  ...
  return (
    <CommandItem
      value={value}
      onSelect={() => onToggle(value, !selected)}
      className="flex items-center gap-2"
    >
      <div className="flex items-center gap-2">
        <Checkbox
          checked={selected}
          className="data-[state=checked]:opacity-100 dark:border-ring"
